    ...
    while(true){ // Simulator loop
        ...
        if(running.pid!=-1 && running.miss_dl(cur_time)){ // In case at this time it missed the deadline
            _log<<"Process P"<<running.pid<<" Missed deadline (and was stopped) at time "<<cur_time<<std::endl;
                            //_log<<">>> k was: "<<running.k<<", deadline was: "<<running.dline<<std::endl;
            ...
            running.pid=-1;
            cur_time--;
            goto sim; // Continue the simulation
            //idle_sec++;
        ...
        if(idle_sec > 0 && !rq.empty()){ // CPU Was idle
            idle_sec=0;
            running=rq.top(); // add new running proc
            rq.pop();
            while(running.miss_dl(cur_time)){ // to check if the process is within deadline
                ...
                fin.push(running);
                //rq.pop();
                if(rq.empty()){ // No runnable process found!
                    running.pid=-1;
                    //cur_time++;
                    goto sim; // Continue simulation
                }
                running=rq.top(); // run this process
                rq.pop();
            }
            ...
            //if(!rq.empty()) rq.pop();
            if(p2.pid==-1 || !running.pr_lt(p2)){ // No SUCH p2
                running.run_time++;
                    //_log<<" >>> Runtime @time: "<<cur_time<<" = "<<running.run_time<<std::endl;
            }
            else{ // p2 exists and has higher priority
                Process temp = rq.top();
        ...
        //_log<<"\tDEBG:  "<<running.pid<<", runtie "<<running.run_time<<", t: "<<running.t<<", curtime"<<cur_time<<std::endl;
        sim:if(running.pid!=-1 && running.run_time==running.t){ // Check if the process finished its execution.
